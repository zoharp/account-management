...
| `TRACE_ADMIN_PASSWORD_EU` | for EU accounts | Fly | The **EU** instance's own `ADMIN_PASSWORD`. Deliberately NOT the same secret as `TRACE_ADMIN_PASSWORD`: the two regional databases never travel together, so one password covering both would mean a single leak opens both regions' admin APIs. There is no fallback — a region with a URL but no password of its own is treated as not configured. |

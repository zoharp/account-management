...
2. **The tenant was pinned to the wrong one.** `PLATFORM_ACCOUNT` is `orcanosdemo`, but this person
   ...
   ⚠️ **0.3.4 disabled that box, but it still sends its value** (`ORCANOS_LOGIN_URL` via
   `/api/auth/config`). Stop sending it and the route falls through to the platform account's own
...
| `deploy.bat` | typecheck → `next build` → **apply master migrations** → **apply tenant migrations** → commit → fetch → push. **It asks nothing at all** — running the script is the approval, and the typecheck/build/migration gates are what protect production. The commit message comes from the command line (`deploy.bat "Fix audit filter"`) or defaults to `Deploy <date> <time>`. Only a *failure* stops and waits; success ends on a 20-second `timeout` so a double-clicked window stays readable without blocking. If the remote is ahead it rebases first and then **re-runs typecheck, build and both migration passes**, because after a rebase the thing being deployed is no longer what was built; a conflict aborts cleanly and pushes nothing. The master migration step (`scripts/apply-master-migrations.mjs`) POSTs each pending `sql/NNN_*.sql` to the Supabase Management API `database/query` route and records the filename in a `schema_migrations` ledger. The tenant step (`scripts/apply-tenant-migrations.mjs`) lists every account from master, parses the project ref out of `vector_db_host` / `db_host`, and re-applies `sql/bootstrap_new_account.sql` to any tenant whose own per-tenant `schema_migrations` does not carry the current file's sha256 hash — the file itself is idempotent (`create ... if not exists`), so re-runs are safe and the hash ledger just skips the work when nothing changed. A tenant with no project host is logged and skipped (an intentional no-DB account); a tenant that errors is reported per-line and the whole step exits nonzero so the deploy fails. Needs `SUPABASE_ORG_ACCESS_TOKEN` (Management API) and the usual `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` (to list accounts) in `.env.local`. |

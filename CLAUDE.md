...
| `deploy.bat` | typecheck → `next build` → **apply master migrations** → commit → fetch → push. **It asks nothing at all** — running the script is the approval, and the typecheck/build/migration gates are what protect production. The commit message comes from the command line (`deploy.bat "Fix audit filter"`) or defaults to `Deploy <date> <time>`. Only a *failure* stops and waits; success ends on a 20-second `timeout` so a double-clicked window stays readable without blocking. If the remote is ahead it rebases first and then **re-runs typecheck, build and migrations**, because after a rebase the thing being deployed is no longer what was built; a conflict aborts cleanly and pushes nothing. The migration step (`scripts/apply-master-migrations.mjs`) POSTs each pending `sql/NNN_*.sql` to the Supabase Management API `database/query` route and records the filename in `schema_migrations` on success — repeat deploys with nothing pending are a fast no-op, and a failed migration blocks the push so code never ships ahead of its schema. Needs `SUPABASE_ORG_ACCESS_TOKEN` in `.env.local`. **Master only — per-tenant `bootstrap_new_account.sql` is still not fanned out.** |

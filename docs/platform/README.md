...
| [`PLATFORM_MODULES_PROPOSAL.md`](PLATFORM_MODULES_PROPOSAL.md) | **Proposal — now largely overtaken** (2026-09-25): module licensing and Training-as-a-module shipped in traceability-matrix (3.23 → 3.46, plus BOM), without the master `user_accounts` part or quiz write-back. Read the handbook §2.7–2.8 for what was built. The module portal: per-module licensing, and adding Training Management to traceability-matrix as a module rather than a panel type. §3.3 and D4 depend on `PLATFORM_AUTH.md`. Its intended destination is the `traceability-matrix` repo, not this one | Consolidation and licensing questions |
